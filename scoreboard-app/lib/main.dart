        ...
        _subscription = eventStream.listen((event) {
          switch (event) {
            case BonsoirDiscoveryServiceFoundEvent():
              ...
              // Resolve immediately for demo purposes
              event.service!.resolve(_discovery!.serviceResolver);
              break;
            case BonsoirDiscoveryServiceResolvedEvent():
              setState(() {
                int index = _discoveredServices.indexWhere((s) => s.name == event.service!.name);
                if (index != -1) {
                  _discoveredServices[index] = event.service!;
                }
              });
              break;
            case BonsoirDiscoveryServiceLostEvent():
              setState(() {
                _discoveredServices.removeWhere((s) => s.name == event.service!.name);
                if (_connectedService?.name == event.service!.name) {
